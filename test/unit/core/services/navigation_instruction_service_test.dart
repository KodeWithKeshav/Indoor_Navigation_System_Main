  ...
  group('NavigationInstructionService', () {
    group('generateInstructions', () {
      ...
      test('should add step numbers to all instructions', () {
        final path = [
          ...
          const Room(id: 'B', floorId: 'f1', name: 'B', x: 10, y: 0, type: RoomType.room),
        ];
        ...
        for (int i = 0; i < instructions.length; i++) {
          expect(instructions[i].message, startsWith('${i + 1}. '),
            reason: 'Step ${i + 1} should start with "${i + 1}. " but was: ${instructions[i].message}');
        }
      });
    });
    ...
    group('Vertical Transitions', () {
      ...
      test('should NOT emit duplicate turn after vertical transition', () {
        ...
        final path = [
          const Room(id: 'A', floorId: 'floor1', name: 'Corridor', x: 0, y: 0, type: RoomType.hallway),
          Room(id: 'S1', floorId: 'floor1', name: 'Stair', x: 5, y: 0, type: RoomType.stairs, connectorId: 'StairA'),
          Room(id: 'S2', floorId: 'floor2', name: 'Stair F2', x: 5, y: 0, type: RoomType.stairs, connectorId: 'StairA'),
          const Room(id: 'D', floorId: 'floor2', name: 'Destination', x: 15, y: 10, type: RoomType.room),
        ];
        final floorLevels = {'floor1': 0, 'floor2': 1};
        ...
        final stairIdx = instructions.indexWhere((i) => i.icon.contains('stairs'));
        expect(stairIdx, greaterThanOrEqualTo(0), reason: 'Should have a stair instruction');
        ...
        if (stairIdx + 1 < instructions.length) {
          ...
          expect(nextInst.icon, isNot(anyOf('left', 'right', 'sharp_left', 'sharp_right')),
    ...
    group('Turn Detection', () {
      ...
      test('should detect 40° bend as a turn (not straight)', () {
        ...
        final path = [
          ...
          const Room(id: 'C', floorId: 'f1', name: 'Target', x: 20, y: 8.39, type: RoomType.room),
        ];
        
        final instructions = service.generateInstructions(path);
        
        final hasTurn = instructions.any((i) => 
          i.icon == 'left' || i.icon == 'right' || 
          i.message.toLowerCase().contains('left') || i.message.toLowerCase().contains('right')
        );
        expect(hasTurn, isTrue, reason: 'A 40° bend should be detected as a turn with 30° threshold');
      });
      ...
      test('should detect U-turn (180° reversal)', () {
        ...
        final path = [
          const Room(id: 'A', floorId: 'f1', name: 'A', x: 0, y: 0, type: RoomType.hallway),
          const Room(id: 'B', floorId: 'f1', name: 'B', x: 10, y: 0, type: RoomType.hallway),
          const Room(id: 'C', floorId: 'f1', name: 'Target', x: 0, y: 0.1, type: RoomType.room),
        ];
        
        final instructions = service.generateInstructions(path);
        
        final hasUturn = instructions.any((i) => 
          i.icon == 'uturn' || i.message.toLowerCase().contains('turn around')
        );
        expect(hasUturn, isTrue, reason: 'A 180° reversal should produce a U-turn instruction');
      });
    });
    ...
    group('Edge Cases', () {
      ...
      test('should merge consecutive straight walks through hallway nodes', () {
        ...
        final walkSteps = instructions.where((i) => i.icon == 'straight' && i.distance > 0).toList();
      ...
      test('should not merge walks across a real turn', () {
        ...
        final path = [
          const Room(id: 'A', floorId: 'f1', name: 'A', x: 0, y: 0, type: RoomType.hallway),
          const Room(id: 'B', floorId: 'f1', name: 'B', x: 10, y: 0, type: RoomType.hallway),
          const Room(id: 'C', floorId: 'f1', name: 'C', x: 10, y: 10, type: RoomType.hallway),
          const Room(id: 'D', floorId: 'f1', name: 'Target', x: 20, y: 10, type: RoomType.room),
        ];
        final corridors = [
          const Corridor(id: 'c1', floorId: 'f1', startRoomId: 'A', endRoomId: 'B', distance: 10),
          const Corridor(id: 'c2', floorId: 'f1', startRoomId: 'B', endRoomId: 'C', distance: 10),
        ...
        final walkSteps = instructions.where((i) => i.icon == 'straight' && i.distance > 0).toList();
        expect(walkSteps.length, greaterThan(1), reason: 'Walks separated by turns should NOT merge');
      });
    });
    ...
    group('First Segment Direction Hint', () {
      test('should provide direction hint even without compass', () {
        final path = [
          const Room(id: 'A', floorId: 'f1', name: 'Start', x: 0, y: 0, type: RoomType.room),
          const Room(id: 'B', floorId: 'f1', name: 'Library', x: 10, y: 0, type: RoomType.library),
        ];
        
        final instructions = service.generateInstructions(path);
        
        // Should contain a "Head towards" instruction
        final hasDirectionHint = instructions.any((i) => 
          i.message.toLowerCase().contains('head towards')
        );
        expect(hasDirectionHint, isTrue, 
          reason: 'Should provide a direction hint even without compass heading');
      });

      test('should provide compass-based turn when heading available', () {
        final path = [
          const Room(id: 'A', floorId: 'f1', name: 'Start', x: 0, y: 0, type: RoomType.room),
          const Room(id: 'B', floorId: 'f1', name: 'End', x: 10, y: 0, type: RoomType.room),
        ];
        ...
        final hasRightTurn = instructions.any((i) => 
          i.message.toLowerCase().contains('right')
        );
        expect(hasRightTurn, isTrue, 
          reason: 'Facing North and needing to go East should produce a Right turn');
      });
    });
  });
}
