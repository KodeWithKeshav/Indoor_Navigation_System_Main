...
void main() {
  test('Admin map usecases return Right from repository', () async {
    ...
    expect(
      await addRoom(
        AddRoomParams(buildingId: 'b1', floorId: 'f1', name: 'R', x: 0, y: 0),
      ),
      const Right(null),
    );
    expect(
      await addCorridor(AddCorridorParams('b1', 'f1', 'r1', 'r2', 1)),
      const Right(null),
    );
    expect(
      (await getCorridors(GetCorridorsParams('b1', 'f1'))).isRight(),
      isTrue,
    );
    expect(
      await deleteRoom(DeleteRoomParams('b1', 'f1', 'r1')),
      const Right(null),
    );
    expect(
      await updateRoom(
