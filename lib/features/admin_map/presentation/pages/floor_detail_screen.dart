...
final roomsProvider = FutureProvider.family<List<Room>, FloorParams>((
  ref,
  params,
) async {
  final getRoomsUseCase = ref.read(getRoomsUseCaseProvider);
  final result = await getRoomsUseCase(
    GetRoomsParams(params.buildingId, params.floorId),
  );
  return result.fold((failure) => throw failure.message, (rooms) => rooms);
});

final corridorsProvider = FutureProvider.family<List<Corridor>, FloorParams>((
  ref,
  params,
) async {
  final getCorridorsUseCase = ref.read(getCorridorsUseCaseProvider);
  final result = await getCorridorsUseCase(
    GetCorridorsParams(params.buildingId, params.floorId),
  );
  return result.fold(
...
class _FloorDetailScreenState extends ConsumerState<FloorDetailScreen> {
  ...
  final TransformationController _transformController =
      TransformationController(); // For scale awareness
  ...
  void _onRoomTap(Room room) async {
    if (_isLinkMode) {
      ...
      if (navState.startRoom != null && navState.endRoom != null) {
        ...
        navNotifier.setStart(room, organizationId: orgId);
        _showToast(
          'Start: ${room.name}. Tap destination (switch floors if needed).',
        );
      } else {
            ...
            onPressed: () async {
              ...
              result.fold(
                (failure) {
                  if (mounted)
                    _showToast(
                      'Failed to link: ${failure.message}',
                      isError: true,
                    );
                },
                (_) {
                  ...
                  ref.invalidate(
                    corridorsProvider(
                      FloorParams(widget.buildingId, widget.floorId),
                    ),
                  );
  ...
  void _addRoom() {
    ...
    final availableTypes = isCampusMap
        ? [
                ...
                RoomType.restroom,
                RoomType
                    .room, // Maybe keep generic 'room' for custom buildings? User said "no indoor things"
                // Let's exclude Room, Stairs, Elevator, Office, Lab
              ]
              .where((t) => t != RoomType.room)
              .toList() // Explicitly remove room
    ...
    showDialog(
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (context, setState) {
          final isConnector =
              selectedType == RoomType.stairs ||
              selectedType == RoomType.elevator ||
              selectedType == RoomType.entrance;
          final isEntrance = selectedType == RoomType.entrance;

          return AlertDialog(
            backgroundColor: darkCardColor,
            title: Text(
              isCampusMap ? 'Add Campus Location' : 'Add Room/Node',
              style: const TextStyle(color: paperWhite),
            ),
            content: Column(
              ...
              children: [
                TextField(
                  ...
                  decoration: InputDecoration(
                    labelText: isCampusMap
                        ? 'Name (e.g. Main Gate, Parking A)'
                        : 'Name (e.g. Room 101, Stair A)',
                ...
                if (isConnector) ...[
                  ...
                  TextField(
                    ...
                    decoration: const InputDecoration(
                      ...
                      helperText:
                          'Must match ID on other floors (e.g. "STAIR-A")',
                ...
                if (isEntrance) ...[
                  ...
                  FutureBuilder(
                    ...
                    builder: (context, snapshot) {
                      ...
                      return DropdownButtonFormField<String>(
                        decoration: const InputDecoration(
                          labelText:
                              'Link to Building ID (Usually this building)',
                          labelStyle: TextStyle(color: Colors.white70),
                        ),
                        dropdownColor: darkCardColor,
                        style: const TextStyle(color: paperWhite),
                        value:
                            connectorIdController.text.isNotEmpty &&
                                validBuildings.any(
                                  (b) => b.id == connectorIdController.text,
                                )
                            ? connectorIdController.text
                            : null,
                  ...
                  const Text(
                    'Matches the Building ID for campus navigation.',
                ...
                onPressed: () async {
                  ...
                  result.fold(
                    (failure) {
                      if (mounted)
                        _showToast(
                          'Failed to add node: ${failure.message}',
                          isError: true,
                        );
                    },
                    (_) {
  ...
  void _deleteSelectedRoom() async {
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Text(
          'Are you sure you want to delete this room/node? This will also break connected corridors.',
    ...
    if (confirmed == true && mounted) {
      ...
      final result = await useCase(
        DeleteRoomParams(widget.buildingId, widget.floorId, _selectedRoomId!),
      );

      result.fold(
        (failure) {
          if (mounted)
            _showToast('Error deleting: ${failure.message}', isError: true);
        },
        (_) {
          setState(() {
          ...
          ref.read(graphServiceProvider).markDirty();
          ref.invalidate(
            roomsProvider(FloorParams(widget.buildingId, widget.floorId)),
          );
          ref.invalidate(
            corridorsProvider(FloorParams(widget.buildingId, widget.floorId)),
          );
  ...
  void _editSelectedRoom() async {
    ...
    final rooms = await ref.read(
      roomsProvider(FloorParams(widget.buildingId, widget.floorId)).future,
    );
        ...
        content: SingleChildScrollView(
          child: Column(
            ...
            children: const [
              ...
              SizedBox(height: 8),
              Text(
                '1. Place Nodes (Small Circles) at corners and intersections.',
                style: TextStyle(color: Colors.white70),
              ),
              Text(
                '2. Place Rooms (Blue Boxes) where they belong.',
                style: TextStyle(color: Colors.white70),
              ),
              Text(
                '3. Link Room → Node to connect a room to the hallway.',
                style: TextStyle(color: Colors.white70),
              ),
              Text(
                '4. Link Node → Node to draw the hallway path.',
                style: TextStyle(color: Colors.white70),
              ),
              SizedBox(height: 16),
              Text(
              ...
              Text(
                '[C101] — [Node] ——— [Node] — [A101]',
                style: TextStyle(fontFamily: 'Courier', color: Colors.white70),
              ),
              Text(
                '             |          |',
                style: TextStyle(fontFamily: 'Courier', color: Colors.white70),
              ...
              Text(
                '• Room: Destination (Office, Class)',
                style: TextStyle(color: Colors.white70),
              ),
              Text(
                '• Hallway/Node: Turning point (Invisible)',
                style: TextStyle(color: Colors.white70),
              ),
              Text(
                '• Stairs/Elevator: Vertical travel',
                style: TextStyle(color: Colors.white70),
              ),
              Text(
                '• Entrance: Building exit/entry',
                style: TextStyle(color: Colors.white70),
              ),
            ],
          ),
                    ...
                    child: InkWell(
                      ...
                      onTap: () {
                        ...
                        if (settings.isVoiceEnabled) {
                          ...
                          if (step.distance > 0) {
                            text +=
                                ', ${step.distance.toStringAsFixed(0)} meters';
                          }
  ...
  Future<void> _importBuildings() async {
    ...
    try {
      ...
      final currentRooms = await ref.read(
        roomsProvider(FloorParams(widget.buildingId, widget.floorId)).future,
      );
  ...
  Widget build(BuildContext context) {
    ...
    ref.listen(navigationProvider, (previous, next) {
      if (_isNavMode &&
          next.pathIds.isNotEmpty &&
          next.pathIds != previous?.pathIds) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          ...
          IconButton(
            icon: Icon(
              _isLinkMode ? Icons.link_off : Icons.link,
              color: _isLinkMode ? electricGrid : Colors.white54,
            ),
            ...
            onPressed: () {
              ...
              _showToast(
                _isLinkMode
                    ? 'Link Mode: ON. Tap two rooms to connect.'
                    : 'Link Mode: OFF',
              );
            },
          ),
          IconButton(
            icon: Icon(
              _isNavMode ? Icons.directions_off : Icons.directions,
              color: _isNavMode ? electricGrid : Colors.white54,
            ),
            ...
            onPressed: () {
              ...
              _showToast(
                _isNavMode
                    ? 'Nav Mode: ON. Select Start then End.'
                    : 'Nav Mode: OFF',
              );
            },
          ),
          ...
          if (_isNavMode)
            IconButton(
              icon: Icon(
                Icons.accessible,
                color: navState.isAccessible ? electricGrid : Colors.white54,
              ),
              tooltip: navState.isAccessible
                  ? 'Accessible Mode: ON'
                  : 'Accessible Mode: OFF',
              onPressed: () {
                ref
                    .read(navigationProvider.notifier)
                    .toggleAccessibility(!navState.isAccessible);
                _showToast(
                  navState.isAccessible
                      ? 'Accessibility OFF'
                      : 'Accessibility ON',
                );
              },
              ...
              data: (rooms) => corridorsAsync.when(
                data: (corridors) {
                  return InteractiveViewer(
                    ...
                    alignment:
                        Alignment.topLeft, // Start at 0,0 so nodes are visible
                    child: SizedBox(
                      width: 15000,
                      height: 15000,
                      child: Stack(
                            ...
                            Positioned(
                              ...
                              child: DraggableRoomNode(
                                ...
                                isSelected:
                                    _selectedRoomId == room.id ||
                                    (_isNavMode &&
                                        navState.startRoom?.id == room.id) ||
                                    (_isNavMode &&
                                        navState.endRoom?.id == room.id),
                                onDragUpdate: (delta) {
                                  setState(() {
                                    final scale = _transformController.value
                                        .getMaxScaleOnAxis(); // Get current zooom
                                    final adjustedDelta = delta / scale;

                                    final current =
                                        _livePositions[room.id] ??
                                        Offset(room.x, room.y);
                                    _livePositions[room.id] =
                                        current + adjustedDelta;
                                  });
                                },
                                onDragEnd: (_) async {
                                  ...
                                  debugPrint(
                                    "Updating room ${room.id} to ${finalOffset.dx}, ${finalOffset.dy}",
                                  );
                                  final useCase = ref.read(
                                    updateRoomUseCaseProvider,
                                  );
                                  final result = await useCase(
                                    UpdateRoomUseParams(
                                      buildingId: widget.buildingId,
                                  ...
                                  result.fold(
                                    (failure) {
                                      if (mounted)
                                        _showToast(
                                          'Failed to update position: ${failure.message}',
                                          isError: true,
                                        );
                                    },
                                    (_) {
                    ...
                    child: Row(
                      children: [
                        ...
                        IconButton(
                          ...
                          onPressed: () =>
                              ref.read(compassProvider.notifier).enableLive(),
...
class DraggableRoomNode extends StatefulWidget {
  ...
  onDragEnd; // Passes nothing (ignored), logic uses state
          ...
          Container(
            ...
            decoration: BoxDecoration(
              ...
              borderRadius: BorderRadius.circular(
                widget.room.type == RoomType.hallway ? 15 : 8,
              ),
            ...
            child: Icon(
              icon ?? Icons.circle,
              color: Colors.white,
              size: widget.room.type == RoomType.hallway ? 0 : 20,
            ),
          ),
  ...
  void paint(Canvas canvas, Size size) {
    final paint = Paint()
      ..color = const Color(0xFF38BDF8)
          .withValues(alpha: 0.1) // electricGrid with low opacity
      ..strokeWidth = 1;
...
class EdgePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    final paint = Paint()
      ..color = const Color(0xFF38BDF8)
          .withOpacity(0.5) // electricGrid with mid opacity for edges
      ..strokeWidth =
          3 // Thicker for visibility
    ...
    for (final corridor in corridors) {
      try {
        ...
        final startPos =
            positions[startRoom.id] ?? Offset(startRoom.x, startRoom.y);
        ...
        // Add 30 to center
        canvas.drawLine(
          startPos + const Offset(30, 30),
          endPos + const Offset(30, 30),
          drawPaint,
        );
      } catch (e) {
        // Room missing
