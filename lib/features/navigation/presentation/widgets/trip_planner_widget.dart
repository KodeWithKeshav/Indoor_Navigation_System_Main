...
class _TripPlannerWidgetState extends ConsumerState<TripPlannerWidget> with SingleTickerProviderStateMixin {
  ...
  List<Room> _getRooms(String buildingId, String? floorId) {
    ...
    // Normal Building
    if (floorId == null) return []; // Require floor selection for normal buildings
  ...
  void _onNavigate() async {
    ...
    if (start != null && end != null) {
      ...
      // Trip Summary Dialog
       final confirm = await showDialog<bool>(
        ...
        builder: (ctx) => AlertDialog(
          ...
          title: const Text('Trip Summary', style: TextStyle(color: Colors.white)),
          content: Column(
             mainAxisSize: MainAxisSize.min,
             crossAxisAlignment: CrossAxisAlignment.start,
             children: [
               _buildSummaryRow(Icons.my_location, "From", start!.name),
               const Padding(padding: EdgeInsets.symmetric(vertical: 4), child: Icon(Icons.arrow_downward, color: Colors.grey, size: 16)),
               _buildSummaryRow(Icons.place, "To", end!.name),
               const SizedBox(height: 16),
               const Text("Ready to start navigation?", style: TextStyle(color: Colors.white70)),
             ],
          ),
          actions: [
      ...
      if (confirm == true) {
          // Save to Recents
          final recentService = ref.read(recentLocationsServiceProvider);
          recentService.addRecentLocation(end); // Save destination
  ...
  void _openSearch(bool isStart) async {
      ...
      if (selectedRoom != null) {
          ...
          final user = ref.read(currentUserProvider);
          final buildings = ref.read(buildingsProvider(user?.organizationId)).value ?? [];
          ...
          try {
             ...
             if (selectedRoom.floorId == 'campus') buildingId = 'campus_${user?.organizationId ?? 'global'}';
          }

          final building = buildings.where((b) => b.id == buildingId).firstOrNull ?? _getCampusBuilding(user?.organizationId);
  ...
  void _resolveLocationContext(Room room, bool isStart) async {
      ...
      try {
          ...
          if (room.floorId == 'campus') buildingId = 'campus_${user?.organizationId ?? 'global'}';
      }
      ...
      if (building != null) {
          // Fetch floors
          final floors = await ref.read(floorsOfBuildingProvider(building.id).future);
  ...
  Widget build(BuildContext context) {
    ...
    final recentsAsync = ref.watch(recentLocationsProvider);
              ...
              return Container(
                ...
                child: Theme(
                  ...
                  child: Column(
                    mainAxisSize: MainAxisSize.min, 
                    children: [
                          // Recent Locations (Horizontal List)
                          recentsAsync.when(
                            data: (recents) {
                               ...
                               return Container(
                                 ...
                                 child: ListView.builder(
                                   ...
                                   itemBuilder: (ctx, idx) {
                                      final loc = recents[idx];
                                      return Padding(
                                        padding: const EdgeInsets.only(right: 8),
                                        child: ActionChip(
                                          backgroundColor: Colors.white10,
                                          labelStyle: const TextStyle(color: Colors.white70, fontSize: 11),
                                          avatar: const Icon(Icons.history, size: 14, color: electricGrid),
                                          label: Text(loc.name),
                                          onPressed: () {
                                              // Auto-fill Destination with recent location
                                              // Need to find Room object. This is async and tricky.
                                              // For now, let's just trigger search with this name?
                                              // Or simpler: Just hint "Search for this?"
                                              _openSearch(false); // Open destination search
                                          },
                                        ),
                                      );
                                   },
                          ...
                          if (_tabController.index == 0) ...[
                             // --- Building Mode ---
                             const Text("Navigates between Main Entrances.", style: TextStyle(color: Colors.white38, fontSize: 11, letterSpacing: 0.5)),
                             ...
                             Center(
                               child: IconButton(
                                 icon: const Icon(Icons.swap_vert_circle, color: electricGrid, size: 32),
                             ...
                             DropdownButtonFormField<Building>(
                              ...
                              _buildLocationSelector(
                                ...
                                onRoomChanged: (r) => setState(() => _startRoom = r),
                                campusBuilding: _getCampusBuilding(user?.organizationId),
           ...
           children: [
             Expanded(
               child: DropdownButtonFormField<Building>(
                  ...
                  items: [
                    DropdownMenuItem(value: campusBuilding, child: const Text('Campus / Outdoor', style: TextStyle(color: paperWhite))),
               ...
               Expanded(
                 ...
                 child: Consumer(
                   builder: (context, ref, _) {
                      final floorsAsync = ref.watch(floorsOfBuildingProvider(selectedBuilding!.id));
                      return floorsAsync.when(
                        data: (floors) => DropdownButtonFormField<Floor>(
                          ...
                          decoration: InputDecoration(
                            labelText: 'Floor',
                            labelStyle: TextStyle(color: paperWhite.withOpacity(0.6)),
                            prefixIcon: const Icon(Icons.layers, color: electricGrid, size: 18),
                            filled: true,
                            fillColor: Colors.black.withOpacity(0.2),
                            border: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide.none),
                          ...
                          style: const TextStyle(color: paperWhite, fontSize: 13),
                          value: selectedFloor,
                          items: floors.map((f) => DropdownMenuItem(value: f, child: Text(f.name, overflow: TextOverflow.ellipsis))).toList(),
             ...
             Expanded(
               ...
               child: DropdownButtonFormField<Room>(
                  ...
                  decoration: InputDecoration(
                    labelText: 'Room',
                    labelStyle: TextStyle(color: paperWhite.withOpacity(0.6)),
                    prefixIcon: const Icon(Icons.door_front_door, color: electricGrid, size: 18),
                    filled: true,
                    fillColor: Colors.black.withOpacity(0.2),
                    border: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide.none),
                    contentPadding: const EdgeInsets.symmetric(horizontal: 12, vertical: 12),
                  ...
                  style: const TextStyle(color: paperWhite, fontSize: 13),
                  value: selectedRoom,
                  items: selectedBuilding == null ? [] : _getRooms(selectedBuilding.id, selectedFloor?.id)
