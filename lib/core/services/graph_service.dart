    ...
    try {
      ...
      for (final building in buildings) {
        ...
        for (final floor in floors) {
          ...
          final corridorsResult = await repository.getCorridors(
            building.id,
            floor.id,
          );
      ...
      print(
        'Graph Built: ${allRooms.length} nodes, ${allCorridors.length} edges.',
      );
      // Debug: Check for vertical connections
      final vertEdges = allCorridors
          .where((c) => c.floorId == 'vertical')
          .length;
      print('Vertical Edges Created: $vertEdges');

      final campusEdges = allCorridors
          .where((c) => c.floorId == 'campus')
          .length;
    ...
    for (final conn in connections) {
      ...
      final fromNodes = allRooms.where((r) {
        ...
        final connectorMatch =
            r.connectorId?.trim().toLowerCase() ==
            fromBldg.trim().toLowerCase();
      ...
      final toNodes = allRooms.where((r) {
        final floorMatch = r.floorId.toLowerCase() == 'ground';
        final connectorMatch =
            r.connectorId?.trim().toLowerCase() == toBldg.trim().toLowerCase();
        return floorMatch && connectorMatch;
      }).toList();
      ...
      if (fromNodes.isEmpty) {
        ...
        final anyConnector = allRooms
            .where(
              (r) => r.connectorId?.toLowerCase() == fromBldg.toLowerCase(),
            )
            .length;
        print(
          ' - Nodes with connectorId "$fromBldg" (any floor): $anyConnector',
        );
      }
      ...
      if (toNodes.isEmpty) {
        ...
        final anyConnector = allRooms
            .where((r) => r.connectorId?.toLowerCase() == toBldg.toLowerCase())
            .length;
